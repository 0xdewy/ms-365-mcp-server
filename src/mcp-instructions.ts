...
function buildGeneralMcpInstructions(opts: McpInstructionsContext): string {
  const parts = [
    ...
    'Files / binary content: for large drive/SharePoint file content, prefer get-download-url to resolve a pre-authenticated URL for out-of-band download. Use download-bytes for authenticated byte reads such as mail attachments, profile photos, Teams hosted content, and meeting recordings. Both tools take relative Microsoft Graph paths, not absolute URLs. For uploads, upload-file-content replaces an existing file item and must not receive a folder id. To create a file inside a folder, use upload-new-file-content for small files or create-new-file-upload-session for large files.',
  ];
